export default [{
  id: 0,
  reviews: [{
    picture: `https://avatarfiles.alphacoders.com/258/258681.jpg`,
    name: `Rita`,
    rating: 5,
    date: new Date(`August 13, 2020`),
    comment: `At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque corrupti quos dolores et quas molestias excepturi sint occaecati cupiditate non provident, similique sunt in culpa qui officia deserunt mollitia animi, id est laborum et dolorum fuga.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/257/257907.jpg`,
    name: `Misha`,
    rating: 4.3,
    date: new Date(`August 24, 2020`),
    comment: `Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus, omnis voluptas assumenda est, omnis dolor repellendus.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/257/257990.jpg`,
    name: `Lena`,
    rating: 2.7,
    date: new Date(`September 2, 2020`),
    comment: `Temporibus autem quibusdam et aut officiis debitis aut rerum necessitatibus saepe eveniet ut et voluptates repudiandae sint et molestiae non recusandae. Itaque earum rerum hic tenetur a sapiente delectus, ut aut reiciendis voluptatibus maiores alias consequatur aut perferendis doloribus asperiores repellat.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/258/258542.jpg`,
    name: `Kirill`,
    rating: 5,
    date: new Date(`September 23, 2020`),
    comment: `Temporibus autem quibusdam et aut officiis debitis aut rerum necessitatibus saepe eveniet ut et voluptates repudiandae sint et molestiae non recusandae. Itaque earum rerum hic tenetur a sapiente delectus, ut aut reiciendis voluptatibus maiores alias consequatur aut perferendis doloribus asperiores repellat.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/257/257999.jpg`,
    name: `Margarita`,
    rating: 1.6,
    date: new Date(`October 6, 2020`),
    comment: `Nor again is there anyone who loves or pursues or desires to obtain pain of itself, because it is pain, but because occasionally circumstances occur in which toil and pain can procure him some great pleasure.`
  }]
}, {
  id: 1,
  reviews: [{
    picture: `https://avatarfiles.alphacoders.com/254/254914.png`,
    name: `Oleg`,
    rating: 1,
    date: new Date(`August 20, 2020`),
    comment: `At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque corrupti quos dolores et quas molestias excepturi sint occaecati cupiditate non provident, similique sunt in culpa qui officia deserunt mollitia animi, id est laborum et dolorum fuga.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/254/254937.png`,
    name: `Dasha`,
    rating: 2,
    date: new Date(`August 29, 2020`),
    comment: `Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus, omnis voluptas assumenda est, omnis dolor repellendus.`
  }]
}, {
  id: 2,
  reviews: [{
    picture: `https://avatarfiles.alphacoders.com/257/257317.png`,
    name: `X-Man`,
    rating: 3,
    date: new Date(`August 2, 2020`),
    comment: `At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque corrupti quos dolores et quas molestias excepturi sint occaecati cupiditate non provident, similique sunt in culpa qui officia deserunt mollitia animi, id est laborum et dolorum fuga.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/257/257970.png`,
    name: `Alina`,
    rating: 5,
    date: new Date(`August 7, 2020`),
    comment: `Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus, omnis voluptas assumenda est, omnis dolor repellendus.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/255/255854.jpg`,
    name: `Flash`,
    rating: 4,
    date: new Date(`August 30, 2020`),
    comment: `Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.`
  }]
}, {
  id: 3,
  reviews: [{
    picture: `https://avatarfiles.alphacoders.com/255/255088.jpg`,
    name: `Kissa`,
    rating: 5,
    date: new Date(`August 2, 2020`),
    comment: `Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/258/258514.jpg`,
    name: `Luba`,
    rating: 4,
    date: new Date(`October 4, 2020`),
    comment: `Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus, omnis voluptas assumenda est, omnis dolor repellendus.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/255/255535.jpg`,
    name: `Olya`,
    rating: 5,
    date: new Date(`October 7, 2020`),
    comment: `At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque corrupti quos dolores et quas molestias excepturi sint occaecati cupiditate non provident, similique sunt in culpa qui officia deserunt mollitia animi, id est laborum et dolorum fuga.`
  }, {
    picture: `https://avatarfiles.alphacoders.com/258/258662.jpg`,
    name: `Vlad`,
    rating: 3,
    date: new Date(`October 15, 2020`),
    comment: `On the other hand, we denounce with righteous indignation and dislike men who are so beguiled and demoralized by the charms of pleasure of the moment, so blinded by desire, that they cannot foresee the pain and trouble that are bound to ensue; and equal blame belongs to those who fail in their duty through weakness of will, which is the same as saying through shrinking from toil and pain. These cases are perfectly simple and easy to distinguish.`
  }]
}];
